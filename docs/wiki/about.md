...
(function(){
  function DiagramPanZoom(pan) {
    ...
    // Compute centering margins for current zoom
    function getMargins() {
      var sw = baseW * zoom, sh = baseH * zoom;
      return {
        l: Math.max(0, (pan.clientWidth - sw) / 2),
        t: Math.max(0, (pan.clientHeight - sh) / 2)
      };
    }

    // Apply layout: inner stays at natural size, transform does visual scaling,
    // margins create scrollable area and center small content
    function applyLayout() {
      ...
      var sw = baseW * zoom, sh = baseH * zoom;
      var m = getMargins();
      inner.style.marginLeft = m.l + 'px';
      inner.style.marginTop = m.t + 'px';
      inner.style.marginRight = Math.max(0, sw - baseW + m.l) + 'px';
      inner.style.marginBottom = Math.max(0, sh - baseH + m.t) + 'px';
      slider.value = Math.round(zoom * 100);
      label.textContent = Math.round(zoom * 100) + '%';
    }

    // Zoom and center (slider, reset, init)
    function applyZoom(newZoom, center) {
      zoom = Math.max(minZoom, Math.min(maxZoom, newZoom));
      applyLayout();
      if (center) {
        var sw = baseW * zoom, sh = baseH * zoom;
        pan.scrollLeft = Math.max(0, (sw - pan.clientWidth) / 2);
        pan.scrollTop = Math.max(0, (sh - pan.clientHeight) / 2);
      }
    }
    ...
    function zoomAt(newZoom, px, py) {
      ...
      var cx = (pan.scrollLeft + px - m.l) / zoom;
      var cy = (pan.scrollTop + py - m.t) / zoom;
      zoom = newZoom;
      applyLayout();
      var nm = getMargins();
      // Reposition scroll so the same content point is under (px, py)
      pan.scrollLeft = nm.l + cx * zoom - px;
      pan.scrollTop = nm.t + cy * zoom - py;
    }

    // Slider: zoom to center
    slider.addEventListener('input', function() {
      applyZoom(parseInt(slider.value) / 100, true);
    });
