...
The equipment to copy from (the _donor_) and the equipment to transfer to (the _target_) must be the same type (so both weapons, both shields, or both bows). Only modifiers that vary between instances of the same weapon (e.g. Shield Guard Up, Long Throw) can be transferred with WST; fixed weapon modifiers (e.g. Improved Flurry Rush, Water Warrior) cannot be transferred.
...
=== "Method 5 (Fuse Entanglement, all versions)"
    ...
    1. Pause and destroy the fusion on the FE parent
